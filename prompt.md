...
### 🎬 JSON 2 - 이미지,영상 프롬프트 (image_data)

아래 형식으로 Step 3의 이미지,영상 애니메이션 프롬프트를 추출하여 출력하세요.
  ...
  "image_data": [
    {
      ...
      "image": "이미지 프롬프트",
      "image_sub": "이미지 프롬프트에서 스타일, 캐릭터 묘사와 같은 반복적으로 삽입되는 부분을 제외한 나머지 부분을 한국어로 번역하여 서술",
      "prompt": "애니메이션 프롬프트",
      "prompt_sub": "애니메이션 프롬프트에서 스타일, 캐릭터 묘사와 같은 반복적으로 삽입되는 부분을 제외한 나머지 부분을 한국어로 번역하여 서술 (움직임 자세히 서술)"
    }
    ...
  ]
}
...
✅ 영상 프롬프트 장면 및 캐릭터 묘사 보완 규칙
- 영상 프롬프트는 이미지 없이 단독으로 사용되므로,
  해당 페이지의 배경/장소/시간대를 1~2문장으로 먼저 설명할 것
- 해당 페이지에 등장하는 캐릭터마다 반드시 아래 항목을 포함할 것:
    · 캐릭터 이름 및 종류 (예: country mouse, city mouse)
    · 캐릭터 고정 레퍼런스의 외형 묘사 (털 색상, 의상, 소품 등)를
      이미지 프롬프트와 동일한 문장으로 삽입
    · 해당 페이지에서의 위치/포즈/행동 상태
- 움직임 묘사는 위 캐릭터/배경 설명 이후에 작성할 것
